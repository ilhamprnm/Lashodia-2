  ...
  return (
    <Carousel responsive={responsive} className=''>
      {products.slice(0, 9).map((product) => {
        ...
        return <div key={product.id} className='p-7 pl-0'>
        <div className='p-4 flex flex-col border rounded-md gap-1'>
          ...
          <div>
            <p className='text-red-500'>${product.price} <s className='text-gray-500 text-sm'>$300</s></p>
          </div>
          <div >
            <img className='h-5' src={(`../images/ratings/rating-${roundedRating}.png`)} alt="" />
          </div>
          <div>
